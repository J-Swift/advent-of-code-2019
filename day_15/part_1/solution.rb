...
class GameWrapper
    ...
    def got_result(result)
        ...
        when :moved
            ...
            DIRECTIONS.each do |dir|
                ...
                if get_space(surrounding_pos) == :unknown && (@best_oxygen_length.nil? || pathfind(@initial_space, surrounding_pos).length < @best_oxygen_length)
                    @unknowns << surrounding_pos
                end
            end
        ...
        when :oxygen
            ...
            @best_oxygen_length = pathfind(@initial_space, @oxygen_position).length
            puts "FOUND OXYGEN: #{@best_oxygen_length}"
        else
    ...
    def pathfind(from, to)
        ...
        while keep_going
            ...
            surrounding_pos.select! { |_, pos| get_space(pos) != :wall && !visited_results[pos] }
    ...
    def get_random_unvisited
        if @unknowns.empty? && !@oxygen_position.nil?
            path = pathfind(@initial_space, @oxygen_position)
            puts '-----------------------'
            puts path.inspect
            puts "FOUND: #{path.size-1}"
            puts '-----------------------'
            exit 1
        end
